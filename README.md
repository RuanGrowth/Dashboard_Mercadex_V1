...
Dockerfile     → build determinístico no Railway (recomendado)
```

> IMPORTANTE: todos esses arquivos precisam ficar na **RAIZ do repositório**,
> não dentro de uma subpasta. Se o `package.json`/`Dockerfile` não estiverem na
> raiz, o Railway não detecta a aplicação e o build falha (erro "Shell").
